...
public boolean sendEmailtls(String subject, String body) {

	  // change accordingly
      final String username = "your@email.com";  
       
      // change accordingly
      final String password = "9kH78Qfze7xz";
	  ...
	  Session session = Session.getDefaultInstance(props, new javax.mail.Authenticator() {
		...
		try {
			Message message = new MimeMessage(session);
			message.setFrom(new InternetAddress("automation@consolidatedhealthplan.com"));
			message.setRecipients(Message.RecipientType.TO, InternetAddress.parse("smurshed@wellfleetinsurance.com"));
			message.setSubject(subject);
			message.setText(body.toString());
			message.setContent(body.toString(), "text/html");
			Transport.send(message);
	...
	public boolean sendEmailAtachmenttls(String subject, String body, String attachment) {
		...
		try {
			...
			// // Recipient's email ID needs to be mentioned.
			String to = "smurshed@wellfleetinsurance.com";

			// Sender's email ID needs to be mentioned
			String from = "automation@consolidatedhealthplan.com";
